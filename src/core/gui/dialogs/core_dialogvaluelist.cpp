...
bool mbCoreDialogValueList::getValueList(const QStringList &all, QStringList &current, const QString &title)
{
    if (title.isEmpty())
        setWindowTitle(Strings::instance().title);
    else
...
void mbCoreDialogValueList::fillForm(const QStringList &all, const QStringList &current)
{
    ui->lsAvailable->clear();
    ui->lsCurrent->clear();
    QStringList available = all;
    Q_FOREACH (const QString &v, current)
    {
        if (!all.contains(v))
            continue;
        ui->lsCurrent->addItem(v);
        int ai = available.indexOf(v);
        if (ai >= 0)
            available.removeAt(ai);
    }
...
void mbCoreDialogValueList::fillData(QStringList &current)
{
    current.clear();
    for (int i = 0; i < ui->lsCurrent->count(); i++)
    {
        QListWidgetItem *item = ui->lsCurrent->item(i);
        current.append(item->data(Qt::DisplayRole).toString());
    }
}
